//! Taking back what the Codex install put down, which used to be
...
//! directory this half did not write, kebacc-switch included: every file is
...
pub fn run(opts: &Options) -> i32 {
    ...
    // The slash commands sit in one directory for both halves, so they belong
    // to whichever one the settings name. Taking them out from under a second
    // install is how uninstalling the copy you did not want breaks the one you
    // did.
    if ours {
        let gone = commands();
        if gone > 0 {
        ...
        // With this half gone the -all pair has nothing to span. The marker
        // above is already off the disk, so this counts what is left.
    ...
    if opts.pool {
        ...
        say(
            &format!(
                "The saved logins are still in {}. Delete them with -Pool.",
                pool.display()
            ),
            Color::Dim,
        );
...
/// true when nothing there mentions this half at all: there is nothing to take
/// out, and the calls below are what write the absence of it.
///
/// Only what runs `kebacc-codex` is read. The Claude half runs a binary of its
/// own, under its own name: its status line and its hooks are none of this
/// uninstaller's business and are never counted here.
///
...
fn settings_point_here(tools: &Path) -> bool {
    ...
    let ours: Vec<String> = ours
        .into_iter()
        .filter(|text| text.contains("kebacc-codex"))
        .map(|text| level(&text))
        .collect();
    if ours.is_empty() {
        return true;
...
fn sweep_reapers() {
    ...
    let mine = format!("kebacc-codex-reap-{}", std::process::id());
    for entry in entries.flatten() {
        let name = entry.file_name();
        let name = name.to_string_lossy();
        if !name.starts_with("kebacc-codex-reap-") || name.starts_with(&mine) {
            continue;
        }
...
fn empty(tools: &Path) -> bool {
    ...
    let ours = [exe.clone(), format!("{exe}.old"), "kebacc-codex.old".into()];
...
fn confirmed(tools: &Path) -> bool {
    say(
        &format!(
            "This removes the Codex slash commands and its half of the session hook, and the binary in {}.",
            tools.display()
        ),
        Color::Plain,
    );
    if tools.join(super::install::CLAUDE_MARKER).is_file() {
        say(
            "kebacc-switch is installed too, and keeps everything of its own.",
            Color::Dim,
        );
    }
...
fn files(tools: &Path) -> (usize, bool) {
    ...
    if let Ok(entries) = std::fs::read_dir(tools) {
        for entry in entries.flatten() {
            let name = entry.file_name();
            let name = name.to_string_lossy();
            if name.starts_with("kebacc-codex.")
                && name.ends_with(".new")
...
/// go, and anything else in the profile stays — the Claude half's line included,
/// which sits under a marker of its own.
fn profiles(tools: &Path) {
    ...
    for path in super::install::profile_paths() {
        ...
        {
            say(
                &format!("Left the kebacc-codex line in {} alone.", path.display()),
                Color::Dim,
            );
            continue;
        }
        ...
        if std::fs::write(&path, &kept).is_ok() {
            say(
                &format!("Took kebacc-codex out of {}", path.display()),
                Color::Green,
            );
        }
    }
...
mod tests {
    ...
    fn the_line_under_the_marker_is_the_one_read_back() {
        let profile = format!(
            "export PATH=/bin\n{}\nkebacc-codex() {{ /a/tools/kebacc-codex \"$@\"; }}\n",
    ...
    fn a_path_is_levelled_to_one_spelling() {
        let one = level("C:\\Users\\a\\.claude-tools\\kebacc-codex.exe");
        let two = level("c:/users/a/.claude-tools/kebacc-codex.exe");
        if cfg!(windows) {
            assert_eq!(one, two);
        } else {
            assert!(one.contains("/kebacc-codex.exe"));
        }
    }
    ...
    fn a_directory_with_only_our_binary_in_it_counts_as_empty() {
        let dir = std::env::temp_dir().join(format!("kebacc-codex-empty-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        let _ = std::fs::write(dir.join(super::super::install::exe_name()), b"x");
        assert!(empty(&dir));
        let _ = std::fs::write(dir.join("something-else"), b"x");
        assert!(!empty(&dir));
