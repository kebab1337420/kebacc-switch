...
rem
rem   install-codex.bat -AutoSwitch

setlocal

set "REPO=kebab1337420/kebacc-switch"
set "SCRIPT=%TEMP%\kebacc-codex-bootstrap.ps1"
...
"%PS%" -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $ProgressPreference='SilentlyContinue'; try{[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12}catch{}; $h=@{'User-Agent'='kebacc-codex-installer'}; $answer=Invoke-RestMethod ('https://api.github.com/repos/'+$env:REPO+'/releases') -Headers $h; $r=@($answer | ForEach-Object {$_} | Where-Object {-not $_.draft -and $_.tag_name -like 'kebacc-codex-v*'}) | Select-Object -First 1; if(-not $r){throw 'No kebacc-codex-v* release has been published yet.'}; $a=@($r.assets | ForEach-Object {$_} | Where-Object {$_.name -eq 'bootstrap.ps1'}) | Select-Object -First 1; if(-not $a){throw ($r.tag_name+' has no bootstrap.ps1 attached to it.')}; Invoke-WebRequest $a.url -OutFile $env:SCRIPT -Headers @{'User-Agent'='kebacc-codex-installer';'Accept'='application/octet-stream'}"
