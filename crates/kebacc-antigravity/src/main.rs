...
fn usage_text() {
    println!("kebacc-antigravity <command> [options]");
    println!();
...
fn dispatch(args: &[String]) -> i32 {
    ...
    if matches!(command.as_str(), "-v" | "--version" | "version") {
        println!("kebacc-antigravity {}", env!("CARGO_PKG_VERSION"));
        return 0;
    }
...
fn parse(tokens: &[String]) -> Result<(String, Options), String> {
    ...
    while index < tokens.len() {
        ...
        match name.as_str() {
            "provider" | "p" => provider = value().ok_or("-Provider needs a name: antigravity.")?,
