...
argument-hint: "-claude|-codex|-ag|-opencode [-Email you@example.com] [-Dir <path>]"
---

Run `~/.claude-tools/kebacc`, or `~/.claude-tools/kebacc.exe` on Windows. This one needs a pool named: `-claude`, `-codex`, `-ag` or `-opencode`.
...
The directory defaults to `~/.kebacc-sessions/<cli>/<address>`; `-Dir` puts it somewhere else. The variable to set is the one that CLI reads: `CLAUDE_CONFIG_DIR` for Claude Code, `CODEX_HOME` for Codex, `XDG_DATA_HOME` for OpenCode, and so on; the command prints the right line for the pool you named. The saved logins stay shared, so an account set up this way is still the same one `list` and `switch` know about.

`XDG_DATA_HOME` is not OpenCode's alone: a shell carrying it sends every program that reads it to that directory, so keep such a shell for the CLI. The command says so itself when the pool is OpenCode.
