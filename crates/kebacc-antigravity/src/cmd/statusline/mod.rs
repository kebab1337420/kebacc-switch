...
fn build(payload: &Value) -> String {
    ...
    let groups: Vec<Vec<Option<String>>> = vec![
        ...
        vec![line.context()],
        vec![
            quota(limits.get("five_hour"), "max"),
            quota(limits.get("seven_day"), "min"),
        ],
        account::segments()
            .into_iter()
...
pub fn gitstat(root: &Path) -> i32 {
    // This one runs inside the detached refresh, which holds no console of its
    // own - so without `hidden` Windows draws git a console window of its own,
    // over whatever the user is looking at, every time the status line redraws.
