# Takes back what the Antigravity installer put down. The saved logins are left
...
# The half kebacc-antigravity shares: which commands are this plugin's, and how to
...
if (-not $Yes) {
    Say "This removes the Antigravity slash commands, and its half of the session hook."
    Say "The kebacc-antigravity binary in $ToolsDir goes with them."
...
$ours = @($script:KebaccMarkers[$pluginId], $exeName, "$exeName.old",
          '.update-antigravity.json', 'update-antigravity.stamp')
...
if (Test-Path -LiteralPath $ToolsDir) {
    ...
    Get-ChildItem -LiteralPath $ToolsDir -Filter 'kebacc-antigravity.*.new' -File -ErrorAction Ignore |
...
# The `kebacc-antigravity` shell function carries this binary's name, so it goes with
...
if (-not $NoProfileEdit -and (Test-Path -LiteralPath $profilePath)) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match '^\s*#\s*kebacc-antigravity account switcher') { $i++; continue }
        $kept += $lines[$i]
    }
    if ($kept.Count -ne $lines.Count) {
        Set-Content -LiteralPath $profilePath -Value $kept -Encoding utf8
        Say "Removed kebacc-antigravity from $profilePath" Green
    }
}

# The session hook. It is one hook for both pools, so this narrows its scope
# when kebacc-antigravity is still armed and only removes it when nothing is left.
...
if (Test-Path -LiteralPath $settingsPath) {
    try {
        ...
        if ($line -and "$($line.Value.command)" -like '*kebacc-antigravity*statusline*') {
...
Say ''
Say 'Uninstalled. The saved logins are still in ~/.kebacc-switch-antigravity-accounts.' DarkGray
exit 0
