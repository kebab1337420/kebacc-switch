//! Taking back what the Antigravity install put down, which used to be
...
//!
//! The login the CLI and the IDE are signed in with is left alone too, in the
//! file and in the credential store both. Uninstalling the switcher is not
//! signing out of Antigravity, and a user who runs this still expects the IDE
//! they were working in to open on the account it was already on.
...
pub fn run(opts: &Options) -> i32 {
    ...
    if ours {
        ...
        super::arm::run("antigravity", true, super::arm::Mode::Drop);
    ...
    say("", Color::Plain);
    say(
        "The Antigravity login you are on was left where it is, in the file and in the credential store.",
        Color::Dim,
    );
    if ours {
        say(
...
///
/// Only what runs `kebacc-antigravity` is read. The Claude half runs a binary
/// of its own, under its own name: its status line and its hooks are none of
/// this uninstaller's business and are never counted here.
///
...
fn confirmed(tools: &Path) -> bool {
    say(
        &format!(
            "This removes the Antigravity slash commands and its half of the session hook, and the binary in {}.",
            tools.display()
        ),
        Color::Plain,
    );
    ...
    say(
        "Saved logins are not touched, and neither is the one you are signed in with.",
        Color::Dim,
    );
...
mod tests {
    ...
    fn a_directory_with_only_our_binary_in_it_counts_as_empty() {
        let dir =
            std::env::temp_dir().join(format!("kebacc-antigravity-empty-{}", std::process::id()));
