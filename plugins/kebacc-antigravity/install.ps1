# Puts the Antigravity half of the switcher on this machine.
#
...
$exeName  = if ($IsWindows -or $env:OS -eq 'Windows_NT') { 'kebacc-antigravity.exe' } else { 'kebacc-antigravity' }
...
if ($Force -or (Test-KebaccShouldReplaceBinary -Entry $entry -Version $version)) {
    if (-not $built) {
        Say "No kebacc binary found. Build it first: cargo build --release -p kebacc-antigravity" Red
        exit 1
    }
    ...
    Say "kebacc-antigravity already put a binary here - using it. -Force replaces it anyway." DarkGray
}
...
# Not every build knows the antigravity pool: an older kebacc-antigravity may have put one
# here from before Antigravity came back into the crate. Saying so now beats five
# slash commands that answer with an unknown provider.
$knows = $false
try { $knows = [bool](& $entry list -Provider antigravity 2>&1 | Select-String -Quiet -SimpleMatch 'Antigravity') } catch {}
if (-not $knows) {
    Say "The binary at $entry does not know the antigravity pool." Red
    Say 'Update kebacc-antigravity, or pass -Binary pointing at a build that does.' Yellow
    exit 1
}
...
if (Test-Path -LiteralPath $commandSource) {
    ...
    Say "Installed the Antigravity slash commands into $commandTarget" Green
}
...
if (-not $NoProfileEdit) {
    ...
    $block  = @(
        $marker
        "function kebacc-antigravity { & `"$entry`" @args }"
    ...
    if ($existing -notmatch [regex]::Escape($marker)) {
        ...
        Say "Added kebacc-antigravity to $profilePath" Green
...
if ($AutoSwitch) {
    ...
    # Only the hooks running kebacc-antigravity are read and rewritten here, and the
    ...
    $scope = Merge-KebaccAutoScope -Existing $armed -Adding 'antigravity'
    ...
    if ($armed -and $scope -ne 'antigravity') {
        Say "  ($armed was already armed, so the one hook now covers both.)" DarkGray
    }
}

Say ''
Say "kebacc-antigravity $version is installed." Green
Say '  /kebacc-add-antigravity       save the Antigravity login you are on' DarkGray
Say '  /kebacc-list-antigravity      what is saved, and its quota' DarkGray
Say '  /kebacc-switch-antigravity    move to another saved login' DarkGray
exit 0
