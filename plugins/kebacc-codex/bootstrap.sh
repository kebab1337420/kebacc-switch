#!/bin/sh
# Installs kebacc-codex on a macOS or Linux machine that has no clone of this
...
api_get() {
    if [ -n "$token" ]; then
        curl -fsSL -H "Authorization: Bearer $token" -H 'User-Agent: kebacc-codex-bootstrap' "$@"
    else
        curl -fsSL -H 'User-Agent: kebacc-codex-bootstrap' "$@"
    fi
}
...
if [ -z "$tag" ]; then
    ...
    # under kebacc-switch-v*, and its releases carry no Codex binary.
...
work=$(mktemp -d 2>/dev/null || mktemp -d -t kebacc-codex)
cleanup() { rm -rf -- "$work"; }
