...
pub fn resolve(id: &str) -> Result<(), String> {
    ...
    match key.as_str() {
        ...
        "antigravity" | "agy" | "google" | "gemini" => Err(format!(
            "'{id}' is the Antigravity pool, which this build does not carry. Install kebacc-antigravity for it; this one switches {PROVIDER_ID} only."
        )),
        _ => Err(format!(
