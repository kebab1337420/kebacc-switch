//! Putting the Codex half on this machine, which used to be `install.ps1` and
//! `install.sh`.
//!
...
pub const COMMANDS: &[(&str, &str)] = &[
    (
        "kebacc-add-codex.md",
        include_str!("../../../../plugins/kebacc-codex/src/commands/kebacc-add-codex.md"),
    ),
    (
        "kebacc-auto-codex.md",
        include_str!("../../../../plugins/kebacc-codex/src/commands/kebacc-auto-codex.md"),
    ),
    (
        "kebacc-doctor-codex.md",
        include_str!("../../../../plugins/kebacc-codex/src/commands/kebacc-doctor-codex.md"),
    ),
    (
        "kebacc-list-codex.md",
        include_str!("../../../../plugins/kebacc-codex/src/commands/kebacc-list-codex.md"),
    ),
    (
        "kebacc-remove-codex.md",
        include_str!("../../../../plugins/kebacc-codex/src/commands/kebacc-remove-codex.md"),
    ),
    (
        "kebacc-switch-codex.md",
        include_str!("../../../../plugins/kebacc-codex/src/commands/kebacc-switch-codex.md"),
    ),
    (
        "kebacc-update-codex.md",
        include_str!("../../../../plugins/kebacc-codex/src/commands/kebacc-update-codex.md"),
    ),
];
...
const DEAD_COMMANDS: &[&str] = &[
    "kebacc-auto-codex-off.md",
    "kebacc-codex-add.md",
    "kebacc-codex-list.md",
    "kebacc-codex-switch.md",
    "kebacc-install-claude.md",
];

/// The marker that says this half is installed here, and the ones the other
/// halves write beside it. A plugin says it is present by leaving its version
/// file in the tools directory: that is the only registry there is, and it is
/// one every half can read without any of them owning it.
pub const VERSION_FILE: &str = ".codex-version";
pub const CLAUDE_MARKER: &str = ".version";
pub const ANTIGRAVITY_MARKER: &str = ".antigravity-version";

const MARKERS: &[&str] = &[CLAUDE_MARKER, VERSION_FILE, ANTIGRAVITY_MARKER];
...
pub const PROFILE_MARKER: &str = "# kebacc-codex account switcher";

pub fn exe_name() -> String {
    format!("kebacc-codex{}", std::env::consts::EXE_SUFFIX)
}
...
pub fn run(opts: &Options) -> i32 {
    ...
    match place(&source, &entry) {
        Ok(true) => say(
            &format!("Installed kebacc-codex into {}", tools.display()),
            Color::Green,
        ),
        Ok(false) => say(
            &format!("kebacc-codex is already at {}", entry.display()),
            Color::Dim,
        ),
        Err(problem) => {
            say(&problem, Color::Red);
    ...
    match commands() {
        Ok(dir) => say(
            &format!("Installed the Codex slash commands into {}", dir.display()),
            Color::Green,
        ),
        Err(problem) => {
            say(&problem, Color::Red);
    ...
    if opts.auto_switch {
        ...
        if !ran(&entry, &["arm", "-Provider", "codex", "-Merge", "-Quiet"]) {
    ...
    say("", Color::Plain);
    say(
        &format!("kebacc-codex {version} is installed."),
        Color::Green,
    );
    say(
        "  /kebacc-add-codex       save the Codex login you are on",
        Color::Dim,
    );
    say(
        "  /kebacc-list-codex      what is saved, and its quota",
        Color::Dim,
    );
    say(
        "  /kebacc-switch-codex    move to another saved login",
        Color::Dim,
    );
    0
}
...
/// `kebacc-codex` as a shell function rather than a directory on the PATH: it
/// is one line to add, and the other half writes a line of its own under a
/// marker of its own, so whichever runs second leaves the first one alone.
    ...
    for path in profile_paths() {
        ...
        if existing.contains(PROFILE_MARKER) {
            ...
            if updated != existing && std::fs::write(&path, &updated).is_ok() {
                say(
                    &format!("Pointed kebacc-codex in {} at the binary", path.display()),
                    Color::Green,
                );
            } else {
                say(
                    &format!("kebacc-codex is already in {}.", path.display()),
                    Color::Dim,
                );
            }
            continue;
        ...
        if std::fs::write(&path, format!("{existing}{block}")).is_ok() {
            say(
                &format!("Added kebacc-codex to {}", path.display()),
                Color::Green,
            );
...
mod tests {
    ...
    fn nothing_outside_this_toolkit_is_swept() {
        ...
        assert!(!ours("kebacc-list-codex.txt"));
    ...
    fn a_powershell_profile_gets_the_powershell_spelling() {
        let line = function_line(Path::new("C:/tools/kebacc-codex.exe"), Path::new("p.ps1"));
        assert!(line.starts_with("function kebacc-codex"));
        let line = function_line(Path::new("/tools/kebacc-codex"), Path::new(".zshrc"));
        assert!(line.starts_with("kebacc-codex()"));
    }
    ...
    fn repointing_replaces_the_line_under_the_marker_and_nothing_else() {
        ...
        let after = repoint(&before, Path::new("/new/kebacc-codex"), Path::new(".zshrc"));
        assert!(after.contains("keep me\n"));
        assert!(after.contains("keep me too"));
        assert!(after.contains("/new/kebacc-codex"));
