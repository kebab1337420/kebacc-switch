#!/bin/sh
# Installs kebacc-switch on a macOS or Linux machine that has no clone of this
...
case "$(uname -s)" in
    ...
    *) red "No published binary for $(uname -s). Clone the repository and run: cargo build --release, then plugins/kebacc-switch/install.sh"; exit 1 ;;
esac
case "$(uname -m)" in
...
api_get() {
    if [ -n "$token" ]; then
        curl -fsSL -H "Authorization: Bearer $token" -H 'User-Agent: kebacc-switch-bootstrap' "$@"
    else
        curl -fsSL -H 'User-Agent: kebacc-switch-bootstrap' "$@"
    fi
}
...
&/g' | grep '"tag_name"' | grep -v '"draft" *: *true' |
    ...
    [ -n "$tag" ] || {
        red "$repo has no published release yet, only drafts."
        exit 1
    }
fi
...
work=$(mktemp -d 2>/dev/null || mktemp -d -t kebacc-switch)
cleanup() { rm -rf -- "$work"; }
...
installer=$(find "$work" -type f -path '*/plugins/kebacc-switch/install.sh' | head -n 1)
[ -n "$installer" ] || { red "The $tag source archive has no plugins/kebacc-switch/install.sh."; exit 1; }
