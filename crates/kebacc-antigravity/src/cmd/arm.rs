...
pub fn run(scope: &str, quiet: bool, mode: Mode) -> i32 {
    ...
    let wanted = match scope.as_str() {
        "off" | "none" | "no" => None,
        "antigravity" | "all" => Some("antigravity".to_string()),
        other => {
            say(
...
/// `antigravity`; `all` is still understood, and so is another half's name,
/// because a hook written back when one binary carried every pool says one of
/// those and has to keep meaning "this pool too" rather than being replaced
/// outright.
fn widen(existing: Option<&str>, adding: &str) -> String {
    ...
    match (had.as_str(), adding) {
        ("claude" | "codex" | "antigravity" | "all", "antigravity" | "all") => "all".to_string(),
...
mod tests {
    ...
    fn disarming_takes_the_mid_task_hook_too() {
        let mut settings = json!({
            "hooks": {
                "SessionStart": [{ "hooks": [
                    { "type": "command", "command": "kebacc-antigravity auto -Provider antigravity -Hook" }
                ] }],
                "PreToolUse": [{ "matcher": "*", "hooks": [
                    { "type": "command", "command": "kebacc-antigravity auto -Provider antigravity -Hook -Midtask" }
                ] }]
            }
        });
        let removed = strip(&mut settings);
    ...
    fn arming_again_only_changes_the_pool_not_the_path() {
        ...
        let next = line(&exe, "antigravity", false);
        assert_eq!(
            next,
            "\"C:/Program Files/tools/kebacc-antigravity.exe\" auto -Provider antigravity -Hook"
        );
    ...
    fn the_mid_task_line_carries_the_flag_and_the_matcher() {
        let command = line("kebacc-antigravity", "antigravity", true);
        assert_eq!(
            command,
            "\"kebacc-antigravity\" auto -Provider antigravity -Hook -Midtask"
        );
        let mut settings = json!({});
    ...
    fn a_windows_path_reaches_the_shell_with_its_separators() {
        ...
        // this was true.
        let raw = "C:\\Users\\me\\.claude-tools\\kebacc-antigravity.exe";
        assert_eq!(
            line(raw, "antigravity", false),
            "\"C:/Users/me/.claude-tools/kebacc-antigravity.exe\" auto -Provider antigravity -Hook"
        );
        let old = format!("{raw} auto -Provider antigravity -Hook");
        assert_eq!(
            line(&exe_of(&old), "antigravity", false),
            "\"C:/Users/me/.claude-tools/kebacc-antigravity.exe\" auto -Provider antigravity -Hook"
        );
    }
