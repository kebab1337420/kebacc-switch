...
/// A short, stable name for an account, taken from its refresh token.
///
/// Antigravity hands out no account id of its own, and the refresh token is the
/// one value that stays put across every access-token refresh. It is a secret,
/// so what is shown and stored is a digest of it and never the token itself.
...
fn cred_hash(creds_raw: Option<&str>) -> String {
    ...
    let Some(token) = crate::live::token_of(&creds) else {
        return "none".into();
    };
    // The access token is deliberately left out: it is refreshed behind this
