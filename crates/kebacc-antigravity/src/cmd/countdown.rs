...
// Antigravity meters per model family rather than per fixed window, so what
// is shown is the family that is furthest gone and the one with the most
// left. The keys stay as they are: they are what the saved cache is written
// under, and renaming them would only orphan it.
const WINDOWS: [(&str, &str); 2] = [("five_hour", "most used"), ("seven_day", "least used")];
