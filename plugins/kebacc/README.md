...
Several logins for Claude Code, Codex, Antigravity and OpenCode, saved on this
...
OpenCode publishes no usage of its own. It is saved, listed, switched and given a session
...
refused for it. Its home variable is `XDG_DATA_HOME`, which no one program owns,
so `use` says out loud that a shell carrying it sends everything else that reads
...
`kebacc arm -ag` (or `-claude`, `-codex`, `-opencode`, `-all`, `off`) is what arms it after
...
`/kebacc-doctor`, `/kebacc-update`. Pass `-ag` (or `-claude`, `-codex`, `-opencode`) as the
...
| `~/.kebacc-switch-opencode-accounts/` | saved OpenCode logins |
...
crates/kebacc/src/main.rs       the entry point, and `-ag` / `-claude` / `-codex` / `-opencode`
