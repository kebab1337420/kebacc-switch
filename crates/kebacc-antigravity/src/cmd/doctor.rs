...
pub fn run(provider: &Provider, opts: &Options) -> i32 {
    ...
    check_credential_store(provider, &report);
    ...
    if !stale.is_empty() {
        if opts.clean {
            ...
            say(
                &format!(
                    "  {} leftover file(s) from an earlier version. Remove with: kebacc-antigravity doctor -Provider {} -Clean",
                    stale.len(),
    ...
    if plain > 0 && !opts.protect {
        say(
            &format!(
                "  {plain} snapshot(s) in plain text. Fix with: kebacc-antigravity doctor -Provider {} -Protect",
                provider::PROVIDER_ID
            ),
            Color::Yellow,
        );
...
/// The CLI's file and the IDE's credential store hold the same login, and are
/// one login only while they agree. A store that has drifted is not an error —
/// the CLI still works — but it leaves the IDE signed in as somebody else, so
/// it is worth saying out loud.
fn check_credential_store(provider: &Provider, report: &Report) {
    if std::env::var_os("KEBACC_SWITCH_NO_KEYRING").is_some() {
        report.warn("KEBACC_SWITCH_NO_KEYRING is set: the IDE's credential store is left alone.");
        return;
    }
    let refresh_of = |raw: &str| -> Option<String> {
        let creds: Value = serde_json::from_str(raw).ok()?;
        live::refresh_token(&creds)
    };
    let in_store = crate::keyring::read().and_then(|raw| refresh_of(&raw));
    let in_file = std::fs::read_to_string(provider.cred_file())
        .ok()
        .and_then(|raw| refresh_of(&raw));
    match (in_file, in_store) {
        (_, None) => report.good("no login in the IDE's credential store"),
        (file, Some(store)) if file.as_deref() == Some(store.as_str()) => {
            report.good("the IDE's credential store holds the same login")
        }
        _ => report.warn(
            "The IDE's credential store holds a different login. Switch again to line the two up.",
        ),
    }
}
...
fn check_settings(report: &mut Report) {
    ...
    let line = settings
        ...
        .unwrap_or_default();
    if line.contains("statusline") && line.contains("kebacc-antigravity") {
    ...
    match midtask.len() {
        0 => report.warn(
            "auto only runs between sessions. Re-arm it to also run mid-task: /kebacc-auto-antigravity",
        ),
        1 => report.good("auto also runs mid-task, before each tool call"),
        count => report.warn(&format!(
            "{count} mid-task hooks run auto. Re-arm it to leave one: /kebacc-auto-antigravity"
        )),
    }
...
pub fn is_auto_command(command: &str) -> bool {
    ...
    let ours = lower.contains("kebacc-antigravity");
...
mod tests {
    ...
    fn a_quoted_path_with_a_space_stays_one_word() {
        let words = quoted_words("\"C:/Program Files/kebacc-antigravity.exe\" statusline");
        assert_eq!(words[0], "C:/Program Files/kebacc-antigravity.exe");
