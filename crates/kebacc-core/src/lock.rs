...
pub fn locked<T>(state_dir: &Path, name: &str, body: impl FnOnce() -> T) -> Result<T, String> {
    let guard = acquire(state_dir, name)?;
    let out = body();
    drop(guard);
    Ok(out)
}
...
#[cfg(windows)]
fn acquire(_state_dir: &Path, name: &str) -> Result<Guard, String> {
...
#[cfg(not(windows))]
fn acquire(state_dir: &Path, name: &str) -> Result<Guard, String> {
    let dir = state_dir.join(format!("{name}.lock"));
