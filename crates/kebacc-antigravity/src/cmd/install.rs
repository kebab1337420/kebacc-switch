...
const MARKERS: &[&str] = &[CLAUDE_MARKER, CODEX_MARKER, VERSION_FILE];
...
/// directory, so a name carrying theirs is left where it is. The -all pair is
/// not ours: whichever half is still standing owns it.
pub fn ours(name: &str) -> bool {
    ...
    name.ends_with(".md") && name.starts_with("kebacc-") && name.contains("antigravity")
}
...
mod tests {
    ...
    fn the_other_halves_commands_are_left_alone() {
        ...
        assert!(!ours("kebacc-update.md"));
        assert!(!ours("kebacc-doctor.md"));
    }
    ...
    fn neither_half_sweeps_the_commands_that_span_every_pool() {
        for (name, _) in ALL_COMMANDS {
            ...
            assert!(
                ...
                "{name} is not ours to ship as an Antigravity-only command"
            );
        }
    }
