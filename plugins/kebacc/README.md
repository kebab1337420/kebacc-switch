...
`switch`, `remove`, `set` and `use` need one: `-claude`/`-cc`, `-codex`/`-cx`, `-antigravity`/`-ag`, `-grok`, `-opencode`/`-oc`.
...
`kebacc arm -ag` (or `-claude`, `-codex`, `-grok`, `-opencode`, `-all`, `off`) is what arms it after
...
`/kebacc-doctor`, `/kebacc-update`. Pass `-ag` (or `-claude`, `-codex`, `-grok`, `-opencode`) as the
...
crates/kebacc/src/main.rs       the entry point, and `-ag` / `-claude` / `-codex` / `-grok` / `-opencode`
