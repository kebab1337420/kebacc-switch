# Installs kebacc-antigravity on a machine that has no clone of this repository and
# no Rust toolchain.
#
...
#
# install-antigravity.bat is a launcher for this file. Running this directly works
# too:
#
...
if (-not ($IsWindows -or $env:OS -eq 'Windows_NT')) {
    ...
    Say '  curl -fsSL https://github.com/kebab1337420/kebacc-switch/releases/download/kebacc-antigravity-v0.2.6/bootstrap.sh | sh' Yellow
    exit 1
}
...
$headers = @{
    'User-Agent' = 'kebacc-antigravity-bootstrap'
...
# assets this script would then look for a Antigravity binary in.
...
$work = Join-Path ([IO.Path]::GetTempPath()) ('kebacc-antigravity-' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $work -Force | Out-Null

try {
    $zip = Join-Path $work 'source.zip'
    Invoke-WebRequest -Uri "https://github.com/$repo/archive/refs/tags/$Tag.zip" -OutFile $zip -Headers @{ 'User-Agent' = 'kebacc-antigravity-bootstrap' }
    ...
    $exe = Join-Path $work 'kebacc-antigravity.exe'
    $binaryHeaders = @{ 'User-Agent' = 'kebacc-antigravity-bootstrap'; 'Accept' = 'application/octet-stream' }
