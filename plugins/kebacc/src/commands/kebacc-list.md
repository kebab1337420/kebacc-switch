...
The numbers come from the API, not from the cache. One block per pool. Repeat the lines as they are; the `*` marks the account in use. This only lists: it never switches, whatever the numbers say. The Grok block carries no numbers: that provider publishes none.
