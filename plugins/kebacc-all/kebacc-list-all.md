...
allowed-tools: Bash(~/.claude-tools/kebacc:*), Bash(~/.claude-tools/kebacc.exe:*), Bash(~/.claude-tools/kebacc-switch:*), Bash(~/.claude-tools/kebacc-switch.exe:*), Bash(~/.claude-tools/kebacc-codex:*), Bash(~/.claude-tools/kebacc-codex.exe:*), Bash(~/.claude-tools/kebacc-antigravity:*), Bash(~/.claude-tools/kebacc-antigravity.exe:*)
---

One binary per pool, and this file is the same file whichever of them installed
it. Run every one of these that is on the machine, `.exe` on Windows. The Claude
binary is `kebacc`; an older install still named `kebacc-switch` is the same
pool, so try that name if `kebacc` is not there.

```
~/.claude-tools/kebacc list -Provider claude -Refresh -Countdown
