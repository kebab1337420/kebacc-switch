...
# kebacc-antigravity carries the Antigravity one. Two binaries, two names, one machine.
...
$script:KebaccMarkers = [ordered]@{ claude = '.version'; antigravity = '.antigravity-version' }
...
$script:KebaccOwnCommands = @{
    claude = @('kebacc-add-claude.md', 'kebacc-list-claude.md', 'kebacc-switch-claude.md',
               ...
               'kebacc-install-antigravity.md')
    antigravity  = @('kebacc-add-antigravity.md', 'kebacc-list-antigravity.md', 'kebacc-switch-antigravity.md',
               'kebacc-remove-antigravity.md', 'kebacc-auto-antigravity.md', 'kebacc-doctor-antigravity.md',
...
$script:KebaccStaleCommands = @{
    ...
    antigravity  = @('kebacc-auto-antigravity-off.md', 'kebacc-antigravity-add.md',
               'kebacc-antigravity-list.md', 'kebacc-antigravity-switch.md',
...
function Sync-KebaccAllCommands {
    ...
    $both = ($Installed -contains 'claude') -and ($Installed -contains 'antigravity')
    $touched = 0
...
# `widen()` in crates/kebacc-antigravity/src/cmd/arm.rs, including what it does with
...
function Read-KebaccArmedScope {
    ...
    if ($Hooks.PSObject.Properties[$Event]) {
        foreach ($group in @($Hooks.$Event)) {
            $mine = @($group.hooks | Where-Object { "$($_.command)" -like '*kebacc-antigravity*auto*' })
...
# `narrow()` in crates/kebacc-antigravity/src/cmd/arm.rs answers the same.
