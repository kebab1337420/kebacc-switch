#!/bin/sh
# Takes kebacc-antigravity back off this machine. The macOS and Linux counterpart of
# uninstall.ps1.
#
...
#   --pool            also delete the saved Antigravity logins
set -eu
...
# Whether kebacc-antigravity is still here decides almost everything below: the
...
removed=0
for name in kebacc-antigravity kebacc-antigravity.old .update-antigravity.json update-antigravity.stamp; do
    if [ -e "$tools_dir/$name" ]; then
        rm -f -- "$tools_dir/$name"
        removed=$((removed + 1))
    fi
done
for half in "$tools_dir"/kebacc-antigravity.*.new; do
    [ -e "$half" ] || continue
    rm -f -- "$half"
    removed=$((removed + 1))
done
...
# This plugin's own names. kebacc-antigravity's commands belong to it and stay until
...
for rc in "$HOME/.zshrc" "$HOME/.bashrc" "$HOME/.profile"; do
    ...
    tmp="$rc.kebacc-tmp"
    grep -vF -e "$marker" -e 'kebacc-antigravity() {' "$rc" > "$tmp"
    mv -f -- "$tmp" "$rc"
    green "Took kebacc-antigravity out of $rc"
done

pool_dir="${KEBACC_SWITCH_ANTIGRAVITY_ACCOUNTS:-$HOME/.kebacc-switch-antigravity-accounts}"
if [ "$pool" = yes ]; then
    if [ -d "$pool_dir" ]; then
        rm -rf -- "$pool_dir"
        yellow "Deleted the Antigravity pool $pool_dir"
    fi
elif [ -d "$pool_dir" ]; then
    dim "The saved Antigravity accounts are still in $pool_dir. Delete them with --pool."
fi
