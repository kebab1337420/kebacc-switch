...
argument-hint: "-claude|-codex|-ag|-grok [-Rank <n>] [-Reserve|-NoReserve] [-FiveHour <pct>] [-SevenDay <pct>] [-OnSwitch <cmd>]"
---
...
Grok takes `-Rank`, `-Reserve` and `-OnSwitch` like any other pool, but not `-FiveHour` or `-SevenDay`: it publishes no usage, so a threshold there would never be read, and the command says so and changes nothing.
