...
Run `~/.claude-tools/kebacc`, or `~/.claude-tools/kebacc.exe` on Windows. Pass the argument through as pool flags (`-ag`, `-claude`, `-codex`, `-opencode`, `-all`). No argument covers every pool.
...
The OpenCode line names the login and stops there, since that provider publishes no usage.
