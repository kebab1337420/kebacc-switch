...
const REPO: &str = match option_env!("KEBACC_SWITCH_RELEASES_REPO") {
    Some(repo) => repo,
    None => "kebab1337420/kebacc-antigravity",
};
const TAG_PREFIX: &str = "kebacc-antigravity-v";
const DEFAULT_INTERVAL_MS: u128 = 24 * 60 * 60 * 1000;
const MAX_BYTES: u64 = 64 * 1024 * 1024;
const DOWNLOAD_SECONDS: u64 = 120;
const RETRY_MS: u128 = 60 * 60 * 1000;

pub const MARKER: &str = ".update-antigravity.json";
const STAMP: &str = "update-antigravity.stamp";
...
pub fn run(opts: &Options) -> i32 {
    ...
    if opts.check {
        say(
            &format!(
                "kebacc-antigravity {} is out. You are on {here}.",
                release.version
            ),
            Color::Yellow,
        );
    ...
    match install(&asset, &here, &release.version) {
        Ok(()) => {
            if !opts.quiet {
                say(
                    &format!("Updated kebacc-antigravity {here} to {}.", release.version),
                    Color::Green,
                );
            }
            0
...
pub fn installed_version() -> Option<String> {
    ...
    let text = std::fs::read_to_string(dir.parent()?.join(".antigravity-version")).ok()?;
    let text = text.trim();
...
fn latest() -> Result<Option<Release>, String> {
    ...
    let mut response = usage::agent()
        .get(&url)
        .header("User-Agent", &format!("kebacc-antigravity/{}", version()))
...
fn install(asset: &Asset, from: &str, to: &str) -> Result<(), String> {
    ...
    lock::locked(lock::UPDATE, || {
        ...
        let fresh = dir.join(format!("kebacc-antigravity.{}.new", std::process::id()));
        ...
        let _ = crate::jsonio::write_text(&dir.join(".antigravity-version"), to);
...
pub fn sweep() {
    ...
    for entry in entries.flatten() {
        ...
        if name.starts_with("kebacc-antigravity.") && name.ends_with(".new") && !recent(&entry) {
...
mod tests {
    ...
    fn a_release_without_a_digest_still_yields_an_asset() {
        let release = serde_json::json!({
            "assets": [{ "name": "kebacc-antigravity-x", "url": "https://api/assets/1" }]
        });
        let asset = asset_of(&release, "kebacc-antigravity-x").expect("asset");
