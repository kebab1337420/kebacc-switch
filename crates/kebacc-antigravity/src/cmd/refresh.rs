...
fn stamp_file() -> PathBuf {
    // Named for this pool: the Claude switcher refreshes on its own clock in
    // the same directory.
    provider::state_dir().join("refresh-antigravity.stamp")
}
...
fn spawn() {
    ...
    command
        .args(["refresh", "-Provider", provider::PROVIDER_ID, "-Spawned"])
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
