...
rem
rem The releases of this half carry the kebacc-antigravity-v prefix: the same
...
rem
rem   install-antigravity.bat -StatusLine -AutoSwitch
rem
...
set "EXE=%TEMP%\kebacc-antigravity-installer.exe"
...
set "ASSET=kebacc-antigravity-x86_64-pc-windows-msvc.exe"
if /i "%PROCESSOR_ARCHITECTURE%"=="ARM64" set "ASSET=kebacc-antigravity-aarch64-pc-windows-msvc.exe"

rem PowerShell 7 when it is here, the one Windows ships with otherwise.
set "PS=powershell"
where pwsh >nul 2>&1 && set "PS=pwsh"

echo Fetching kebacc-antigravity...
...
"%PS%" -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $ProgressPreference='SilentlyContinue'; try{[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12}catch{}; $h=@{'User-Agent'='kebacc-antigravity-installer'}; if($env:GITHUB_TOKEN){$h['Authorization']='Bearer '+$env:GITHUB_TOKEN}; $answer=Invoke-RestMethod ('https://api.github.com/repos/'+$env:REPO+'/releases') -Headers $h; $r=@(@($answer) | Where-Object {-not $_.draft -and $_.tag_name -like 'kebacc-antigravity-v*'}) | Select-Object -First 1; if(-not $r){exit 2}; $a=@(@($r.assets) | Where-Object {$_.name -eq $env:ASSET}) | Select-Object -First 1; if(-not $a){throw ($r.tag_name+' has no '+$env:ASSET+' attached to it.')}; $d=$h.Clone(); $d['Accept']='application/octet-stream'; Invoke-WebRequest $a.url -OutFile $env:EXE -Headers $d"
...
:unpublished
echo.
echo No kebacc-antigravity-v* release has been published yet, so there is nothing to install.
