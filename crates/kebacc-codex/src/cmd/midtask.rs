...
/// `crates/kebacc-codex/` — is not a switcher call, and used to cost every
...
mod tests {
    ...
    fn working_on_the_switcher_still_arms_the_check() {
        assert!(!about_us(
            r#"{"tool_name":"Bash","tool_input":{"command":"grep -rn kebacc crates/kebacc-codex/src"}}"#
        ));
        assert!(!about_us(
            r#"{"tool_name":"Edit","tool_input":{"file_path":"crates/kebacc-codex/src/cmd/midtask.rs"}}"#
        ));
    }
    ...
    fn the_windows_binary_counts_too() {
        assert!(about_us(
            r#"{"tool_name":"Bash","tool_input":{"command":"\"C:\\Users\\x\\.claude-tools\\kebacc-codex.exe\" doctor"}}"#
        ));
    }
    ...
    fn the_other_halves_count_too() {
        assert!(about_us(
            r#"{"tool_name":"Bash","tool_input":{"command":"~/.claude-tools/kebacc list"}}"#
        ));
        assert!(about_us(
