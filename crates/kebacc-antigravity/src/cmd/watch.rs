...
//! `session-antigravity.beat` every time they run, and the watcher gives up
//! once that stamp goes cold, which is what a closed CLI looks like from here.
//! Failing that, it dies of old age. The names carry the half so a Claude
//! watcher and an Antigravity watcher sharing `~/.kebacc-switch` do not read
//! each other's stamps.
...
const BEAT: &str = "watch-antigravity.beat";
...
const STOP: &str = "watch-antigravity.stop";
/// The last time a hook ran, which is the last proof a session exists.
const SESSION_BEAT: &str = "session-antigravity.beat";
...
mod tests {
    ...
    fn a_missing_stamp_reads_as_ancient() {
        assert_eq!(age("kebacc-antigravity-no-such-stamp"), Duration::MAX);
    }
    ...
    fn unique_stamp(label: &str) -> (PathBuf, PathBuf) {
        let dir = std::env::temp_dir().join(format!(
            "kebacc-antigravity-watch-{label}-{}-{}",
            std::process::id(),
            now_ms()
        ));
