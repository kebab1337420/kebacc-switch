---
description: Turn the auto-switch on or off, any time, no conditions
allowed-tools: Bash(~/.claude-tools/kebacc:*), Bash(~/.claude-tools/kebacc.exe:*)
argument-hint: "[on|off|claude|codex|antigravity|all]"
---

Flip the auto-switch. Argument: `$ARGUMENTS`

Run `~/.claude-tools/kebacc`, or `~/.claude-tools/kebacc.exe` on Windows:

```
~/.claude-tools/kebacc arm -Provider <scope>
```

`<scope>` comes from the argument:

- `off`: `off`
- `claude`, `codex`, `antigravity`, `all`: that scope
- `on`, empty, or no argument: `all` when nothing is armed, `off` when
  something is. `~/.claude-tools/kebacc doctor` says which it is.

The command writes both of its hooks in `~/.claude/settings.json` itself, leaving every other hook alone. It never changes the account in use on its own.

Print the one line it prints and stop.
